...
CGameApp::CGameApp()
{
	...
	m_pPlayer		= NULL;
	m_LastFrameRate = 0;
	//tempPlayerPos=   Vec2(400,200);
	//tempPlayerPos1 = Vec2(800, 300);
}
	...
	switch (Message)
	{
		...
		case WM_KEYDOWN:
			...
			case VK_RETURN:
				...
				break;
			case VK_CONTROL:
				MessageBox(hWnd, "Game is saved.", "Save Game", MB_OK);
				SaveGame();
				break;
			case 0x4C:
				MessageBox(hWnd, "Game is loaded.", "Load Game", MB_OK);
				LoadGame();
				break;
...
bool CGameApp::BuildObjects()
{
	...
	if(!m_imgBackground.LoadBitmapFromFile("data/background3.bmp", GetDC(m_hWnd)))
		return false;
...
void CGameApp::SetupGameState()
{
	...
	m_pPlayer->HeartPositions();

	m_pPlayer->CratesPositions();

    m_pPlayer->SpawnEnemies();

	m_pPlayer->LivesSpawn();

	//m_pPlayer->EnemiesShootings();
}

void CGameApp::SaveGame()
{
	 std:: ofstream myfile("Save-Load.txt");
	 myfile << m_pPlayer->Position().x<<" "<<m_pPlayer->Position().y<<"\n";
	 myfile.close();
}

void CGameApp::LoadGame()
{
	std::ifstream myfile("Save-Load.txt");
	double posx, posy;
	myfile >> posx>>posy;
	m_pPlayer->Position() = Vec2(posx, posy);
...
void CGameApp::collisionDetection() {
	static UINT		fTimer;
	//fTimer = SetTimer(m_hWnd, 1, 250, NULL);
	/*if ((m_pPlayer->Position().x+m_pPlayer->GetWidth()/2 >= m_pPlayer1->Position().x - m_pPlayer1->GetWidth() / 2 &&
		m_pPlayer->Position().y+m_pPlayer->GetHeight()/2>=m_pPlayer1->Position().y-m_pPlayer1->GetHeight()/2&&
		m_pPlayer->Position().x-m_pPlayer->GetWidth()/2 <= m_pPlayer1->Position().x + m_pPlayer1->GetWidth() / 2)&&
		m_pPlayer->Position().y - m_pPlayer->GetHeight() / 2 <= m_pPlayer1->Position().y + m_pPlayer1->GetHeight() / 2)
	{
		fTimer = SetTimer(m_hWnd, 1, 250, NULL);
		fTimer1 = SetTimer(m_hWnd, 2, 250, NULL);
		m_pPlayer->Explode(); 
		m_pPlayer1->Explode();
	}*/
	if (m_pPlayer->crateCollision() == true)
	{
		fTimer = SetTimer(m_hWnd, 1, 75, NULL);
		m_pPlayer->Explode();
	}

	if (m_pPlayer->EnemyBulletCollision() == true)
	{
		fTimer = SetTimer(m_hWnd, 1, 75, NULL);
		m_pPlayer->Explode();
	}

	if (m_pPlayer->EnemySpaceshipCollision() == true)
	{
		fTimer = SetTimer(m_hWnd, 1, 75, NULL);
		m_pPlayer->Explode();
	}

	m_pPlayer->BulletEnemySpaceshipCollision();

	m_pPlayer->bulletcrateCollision();

	m_pPlayer->LivesCollision();
...
void CGameApp::FrameAdvance()
{
	...
	if ( m_LastFrameRate != m_Timer.GetFrameRate() )
	{
		...
		sprintf_s( TitleBuffer, _T("Game : %s    Score: %d "), FrameRate, m_pPlayer->GetScore());
